...
![image-20220716221909268](https://s2.loli.net/2022/07/16/u4PwJjbIzds2YLV.png)

#### 数据写入

事务更新 comment_subject，comment_index，comment_content 三张表，其中 content 属于非强制需要一致性考虑的。可以先写入 content，之后事务更新其他表。即便 content 先成功，后续失败，也仅存在一条 ghost 数据。
...
Tips: 是不是可以使用 Graph 存储？DGraph、HugeGraph 类似的图存储思路。

#### 索引与内容分离

![image-20220717182945045](C:\Users\Ther\AppData\Roaming\Typora\typora-user-images\image-20220717182945045.png)

comment_index: 评论楼层的索引组织表，实际并不包含内容。
comment_content: 评论内容的表，包含评论的具体内容。其中 comment_index 的 id 字段和 comment_content 是 1 对 1 的关系，这里面包含几种设计思想。
...
![image-20220717225941315](https://s2.loli.net/2022/07/17/7ArYIHQ21il6Gtg.png)

comment_subject_cache: 对应主题的缓存，value 使用 protobuf 序列化的方式存入。早期使用 memcache 来进行缓存，因为 Redis 早期单线程模型，吞吐能力不高。
...
> 为什么判定 key 存在后才能 add？
>
> 若直接 add，则可能 key 已不存在，结果就是只 add 了要追加的一部分。
...
![image-20220717230755891](https://s2.loli.net/2022/07/17/y4qRxKWImXkAvpg.png)
...
![image-20220717231201889](https://s2.loli.net/2022/07/17/sHDIyce7glU1V5E.png)
